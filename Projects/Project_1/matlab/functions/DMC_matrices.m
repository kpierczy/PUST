...
function [K, Mp] = DMC_matrices(step_response, DMC_struct)
...
for i = 1:DMC_struct.Nu
    M(i:DMC_struct.N,i) = step_response(1:DMC_struct.N+1-i);
end
...
for i = 1:D-1
   for j = 1:DMC_struct.N
       if(i+j>D)
            Mp(j,i) = step_response(D);
       else    
            Mp(j,i) = step_response(i+j);
       end
   end
end

for i = 1:D-1
    Mp(:,i) = Mp(:,i) - ones(DMC_struct.N,1) .* step_response(i);
end
