...
for i = 3 : 1 : size(lst)
    ...
    name_plot_file = regexprep(fullfile('data\exercise_7', lst(i).name),'\','/');
    name_plot_label = lst(i).name(1 : size(lst(i).name,2) - 4);
    
    
    fprintf(fid1, format1, name_plot_file);
    fprintf(fid1, format2, name_plot_label, name_file_caption);
